...
%% Graphs %%
x_me = [0, L_start,(L_start+L_extra), L_co, (L_co+L_extra), L_stroke, (L_stroke+L_extra), L_stroke2, (L_stroke2+L_extra), L_co2];
y_me = [0, E_md_start,E_md_co, E_md_co, E_md_ro, E_md_ro, E_md_rc, E_md_rc, E_md_pc, E_md_pc];

coder.extrinsic('plot')

plot(x_me,y_me,'-', 'LineWidth', 1.5)
grid on
hold on
ylim([-30e3 70e3])
...
set(gca,'YTick',[-30e3 -20e3 -10e3 0 10e3 20e3 30e3 40e3 50e3 60e3 70e3] ); %This are going to be the only values affected.
set(gca,'YTickLabel',[-30 -20 -10 0 10 20 30 40 50 60 70] ); %This is what it's going to appear in those places.
title('Required Motor Mechanical Energy for each Operational Scenario')
xlabel('Stroke [mm]')
ylabel('Energy [kJ]')
set(gca,'FontSize',20)
legend('\mu_{stat} = 0.1', '\mu_{stat} = 0.2', '\mu_{stat} = 0.3')
end 
